...
def notes_edit(note_id):
    ...
    if form.validate_on_submit():
        ...
        new_tag_names = set(form.tags.data)
        current_tag_names = {tag.name for tag in note.tags}

        tags_to_remove = [tag for tag in note.tags if tag.name not in new_tag_names]
        ...
        for tag in tags_to_remove:
            if not db.session.execute(db.select(models.Note).where(models.Note.tags.any(id=tag.id))).first():
